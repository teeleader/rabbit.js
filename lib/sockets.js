...
PubSocket.prototype.publish = function(topic, chunk, encoding) {
  ...
  // if it is a function we'll just pass in our data/chunk and let the function determine what the topic is
...
WorkerSocket.prototype.connect = function(source, callback) {
  ...
  function consume() {
    return ch.consume(source, function(msg) {
      if (msg && !self.options.bypassUnackedArray) {
        self.unacked.push(msg);
      }

      if (self.options.consumeRawMsg === true) {
        self.push(msg);
      } else {
        self.push(msg && msg.content);  
      }
  ...
  if (this.options.noCreate) {
    ...
    .then(function(ok) {
      if (!self.options.exchange || !self.options.topic) {
        return;
      }

      // bind the queue
      return ch.bindQueue(source, self.options.exchange, self.options.topic, self.options.bindArgs || {});
    })
    .then(consume);
  }
};

WorkerSocket.prototype.ack = function(msg) {
  if (!msg) {
    msg = this.unacked.shift();
  }
  if (!msg) {
...
WorkerSocket.prototype.requeue = function(msg) {
    if (!msg) {
      msg = this.unacked.shift();
    }
    if (!msg) {
...
WorkerSocket.prototype.discard = function(msg) {
    if (!msg) {
      msg = this.unacked.shift();
    }
    if (!msg) {
